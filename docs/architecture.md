...
一个**纯本地**的 Web 工具，用于查看与回顾 Claude Code / Codex / Kimi 的历史 session。读取本机 `~/.claude/projects/`、`~/.codex/sessions/` 与 `~/.kimi-code/sessions/` 下的 JSONL 文件，解析后通过浏览器展示：
...
- 不写入或修改任何 Claude Code / Codex / Kimi 的原始数据（`~/.claude/`、`~/.codex/`、`~/.kimi-code/` 全程只读）
...
│   │   ├── parser.ts               # Claude/Codex/Kimi JSONL → SessionDetail + ToolCallPair 配对
│   │   ├── pricing.ts              # 转发至 lib/pricing
│   │   ├── routes.ts               # /api/* 路由分发
│   │   ├── scanner.ts              # 扫描 ~/.claude/projects/、~/.codex/sessions/ 与 ~/.kimi-code/sessions/
...
~/.kimi-code/sessions/<hash>/<session>/agents/<agent>/wire.jsonl
        │
        ▼
  scanner.ts ── 列项目 / session 文件 + sub-agent 子目录（Claude / Kimi）
        │
        ▼
